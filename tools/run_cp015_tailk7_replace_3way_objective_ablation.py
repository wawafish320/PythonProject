...
RUN_TAG = "20260402_3way_objective"
OUT_ROOT = ROOT / "debug_output" / f"_tmp_cp015_tailk7_replace_3way_objective_ablation_{RUN_TAG}"
MODEL_ROOT = ROOT / "models" / f"__tmp_cp015_tailk7_replace_3way_objective_ablation_{RUN_TAG}"
...
PRIMARY_CASES: Tuple[Tuple[str, float, float, float], ...] = (
    ("e2x60_3way_eq", 1.0, 1.0, 1.0),
    ("e2x60_3way_arm125", 1.0, 1.25, 1.0),
)
CONDITIONAL_CASE: Tuple[str, float, float, float] = ("e2x60_3way_arm150", 1.0, 1.5, 1.0)
...
def tail_stats(log_path: Path) -> Dict[str, Any]:
    ...
    keys = (
        ...
        "dir_arm_base",
        "dir_else_base",
        "direct_pose_3way_active",
        "direct_pose_loss_3way_w_leg",
        "direct_pose_loss_3way_w_arm",
        "direct_pose_loss_3way_w_else",
        ...
        "dir_group_norm_arm_raw",
        "dir_group_norm_else_raw",
        "dir_group_norm_leg_ema",
        "dir_group_norm_nonleg_ema",
        "dir_group_norm_arm_ema",
        "dir_group_norm_else_ema",
        "dir_group_norm_w_leg",
        "dir_group_norm_w_nonleg",
        "dir_group_norm_w_arm",
        "dir_group_norm_w_else",
    )
...
def run_case(case_name: str, w_leg: float, w_arm: float, w_else: float) -> Dict[str, Any]:
    ...
    make_generated_config(
        ...
        cfg_json,
        {
            ...
            "direct_pose_loss_3way_enable": True,
            "direct_pose_loss_3way_w_leg": float(w_leg),
            "direct_pose_loss_3way_w_arm": float(w_arm),
            "direct_pose_loss_3way_w_else": float(w_else),
...
def build_markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        "# cp015 tailk7 replace 3-way objective ablation",
        "",
        "## Code Audit",
        "",
        f"- current e2x60 config 3-way keys: `enable={audit['current_e2x60_config'].get('direct_pose_loss_3way_enable')}` / "
        f"`w_leg={audit['current_e2x60_config'].get('direct_pose_loss_3way_w_leg')}` / "
        f"`w_arm={audit['current_e2x60_config'].get('direct_pose_loss_3way_w_arm')}` / "
        f"`w_else={audit['current_e2x60_config'].get('direct_pose_loss_3way_w_else')}`",
        f"- current e2x60 log 3-way active: `{audit['current_e2x60_log_tail'].get('direct_pose_3way_active')}`",
        "",
        "## Policy",
        "",
        ...
        f"- conditional arm150 executed: `{summary['policy']['conditional_case_executed']}`",
        "",
    ...
    for case_name in summary["cases_in_order"]:
        ...
        lines.extend(
            [
                f"### {case_name}",
                "",
                f"- 3-way active: `{tail.get('direct_pose_3way_active')}`",
                f"- weights leg/arm/else: `{tail.get('dir_group_norm_w_leg')}` / `{tail.get('dir_group_norm_w_arm')}` / `{tail.get('dir_group_norm_w_else')}`",
                f"- dir_leg / dir_arm / dir_else: `{tail.get('dir_leg_base')}` / `{tail.get('dir_arm_base')}` / `{tail.get('dir_else_base')}`",
                f"- gnorm 3-way active: `{tail.get('dir_group_norm_3way_active')}`",
                f"- gnorm leg/arm/else: `{tail.get('dir_group_norm_leg')}` / `{tail.get('dir_group_norm_arm')}` / `{tail.get('dir_group_norm_else')}`",
                f"- gnorm ema leg/arm/else: `{tail.get('dir_group_norm_leg_ema')}` / `{tail.get('dir_group_norm_arm_ema')}` / `{tail.get('dir_group_norm_else_ema')}`",
                "",
            ]
        )
    lines.extend(
...
def main() -> int:
    ...
    for case_name, w_leg, w_arm, w_else in PRIMARY_CASES:
        log(f"=== running 3-way case {case_name} (leg={w_leg}, arm={w_arm}, else={w_else}) ===")
        payload = run_case(case_name, w_leg, w_arm, w_else)
        payload["delta_vs_current_70a"] = compare_slots(payload["metrics"], current_70a_metrics)
    ...
    if should_run_conditional:
        case_name, w_leg, w_arm, w_else = CONDITIONAL_CASE
        log(f"=== running conditional 3-way case {case_name} (leg={w_leg}, arm={w_arm}, else={w_else}) ===")
        payload = run_case(case_name, w_leg, w_arm, w_else)
        payload["delta_vs_current_70a"] = compare_slots(payload["metrics"], current_70a_metrics)
        payload["delta_vs_current_replace_e2x60"] = compare_slots(payload["metrics"], current_replace_e2x60_metrics)
        payload["delta_vs_baseline_replace"] = compare_slots(payload["metrics"], baseline_replace_metrics)
    ...
    summary = {
        ...
        "policy": {
            ...
            "primary_cases": [
                {"name": name, "w_leg": w_leg, "w_arm": w_arm, "w_else": w_else}
                for name, w_leg, w_arm, w_else in PRIMARY_CASES
            ],
            "conditional_case": {
                "name": CONDITIONAL_CASE[0],
                "w_leg": CONDITIONAL_CASE[1],
                "w_arm": CONDITIONAL_CASE[2],
                "w_else": CONDITIONAL_CASE[3],
            },
        ...
        "code_audit": {
            "current_e2x60_config": {
                ...
                "direct_pose_loss_3way_enable": current_e2x60_config.get("direct_pose_loss_3way_enable"),
                "direct_pose_loss_3way_w_leg": current_e2x60_config.get("direct_pose_loss_3way_w_leg"),
                "direct_pose_loss_3way_w_arm": current_e2x60_config.get("direct_pose_loss_3way_w_arm"),
                "direct_pose_loss_3way_w_else": current_e2x60_config.get("direct_pose_loss_3way_w_else"),
            },
