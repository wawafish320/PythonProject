#!/usr/bin/env python3
"""
Run Phase2-A leg-head capacity matrix:
  A0 = linear
  A1 = residual MLP (hidden=16)
  A2 = residual MLP (hidden=32)

Training/eval protocol follows Step7 best branch defaults:
  - e3, lr=3e-4 base config
  - 3 seeds
...
CASES: Dict[str, CaseSpec] = {
    ...
    "A1": CaseSpec(
        ...
        config_overrides={
            ...
            "direct_pose_leg_head_hidden": 16,
            "direct_pose_leg_head_res_alpha_init": 0.0,
        },
    ),
    "A2": CaseSpec(
...
STEP7_CASE_OVERRIDES: Dict[str, Dict[str, Any]] = {
    # Step7 best branch default (R2-like): stopgrad_main=false, detach_feat=true.
    "r2": {
        "direct_pose_leg_stopgrad_main": False,
        "direct_pose_leg_detach_feat": True,
    },
    # Alternate (R3-like): both off.
    "r3": {
        "direct_pose_leg_stopgrad_main": False,
        "direct_pose_leg_detach_feat": False,
    },
    # Keep whatever base-config already has.
    "base": {},
}
...
def _aggregate_by_case(rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, float]]:
...
def _phase2a_gate(
    ...
    *,
    ...
    for k in ("A1", "A2"):
        ...
        # Table should be optional: gain must stay marginal.
    ...
    decision = "continue_readout_capacity" if passed else "fallback_h2a_adapter_or_bypass"
    if not passed:
        # If no-table still fails while fixed-table remains strong, this is table-dependent pseudo-fix.
        pseudo_fix_cases: List[str] = []
        for k in ("A1", "A2"):
            ck = checks.get(k, {})
            if not isinstance(ck, dict):
                continue
            mt = ck.get("metrics", {}) if isinstance(ck.get("metrics", {}), dict) else {}
            fixed_sign = float(case_stats.get(k, {}).get("fixed_sign_mean", float("nan")))
            fixed_med = float(case_stats.get(k, {}).get("fixed_med_mean", float("nan")))
            nt_sign = float(mt.get("no_table_sign_mean", float("nan")))
            nt_med = float(mt.get("no_table_med_mean", float("nan")))
            if math.isfinite(fixed_sign) and math.isfinite(fixed_med) and math.isfinite(nt_sign) and math.isfinite(nt_med):
                if (fixed_sign >= no_table_sign_mean_min and fixed_med <= no_table_med_mean_max) and (
                    nt_sign < no_table_sign_mean_min or nt_med > no_table_med_mean_max
                ):
                    pseudo_fix_cases.append(k)
        if pseudo_fix_cases:
            decision = "reject_table_dependent_pseudo_fix"
    return {
        "decision": decision,
        "passed_cases": passed,
...
def _write_md(summary: Dict[str, Any], out_md: Path) -> None:
    ...
    gate = dict(summary.get("phase2a_gate", {}) or {})
    lines: List[str] = []
    lines.append("# Phase2-A leg head capacity summary")
    lines.append("")
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Run Phase2-A leg-head capacity matrix (A0/A1/A2 x 3-seed).")
    ...
    ap.add_argument("--cases", type=str, default="A0,A1,A2")
    ...
    for case_key in requested_cases:
        ...
        for seed in requested_seeds:
            run_name = f"h1_10p3a_phase2a_{case.key.lower()}_seed{seed}_e3_lr3e4_{args.date_tag}"
    ...
    case_stats = _aggregate_by_case(rows)
    phase2a_gate = _phase2a_gate(
        case_stats,
    ...
    out_json = out_root / "phase2a_leghead_capacity_summary.json"
    out_md = out_root / "phase2a_leghead_capacity_summary.md"
