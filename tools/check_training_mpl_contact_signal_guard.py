...
This guard prevents accidental regressions after the whitebox/fallback removal:
...
- the retired whitebox helper must not reappear in the basetrain runtime
"""
...
WHITEBOX_HELPER = "_contact_meas_" + "whitebox"
...
def _validate_source(src_path: Path) -> list[str]:
    ...
    if rollout_fn is not None:
        step_inputs_calls = _find_calls(rollout_fn, STEP_INPUTS_METHOD)
        whitebox_calls = _find_calls(rollout_fn, WHITEBOX_HELPER)
        source_branch_lines = _find_if_lines_with_name(rollout_fn, "trainbase_contacts_source")

        if len(step_inputs_calls) == 0:
            errs.append(
                f"{src_path}:{rollout_fn.lineno}: `{ROLLOUT_METHOD}` must call `{STEP_INPUTS_METHOD}` to resolve contact signal"
            )
        elif len(step_inputs_calls) > 1:
            errs.append(
                f"{src_path}:{_fmt_lines(step_inputs_calls)}: `{ROLLOUT_METHOD}` should keep a single `{STEP_INPUTS_METHOD}` call"
            )

        if whitebox_calls:
            errs.append(
                f"{src_path}:{_fmt_lines(whitebox_calls)}: `{ROLLOUT_METHOD}` must not call `{WHITEBOX_HELPER}` directly"
            )
    ...
    if step_inputs_fn is not None:
        ...
        step_inputs_whitebox_calls = _find_calls(step_inputs_fn, WHITEBOX_HELPER)
        ...
        if step_inputs_whitebox_calls:
            errs.append(
                f"{src_path}:{_fmt_lines(step_inputs_whitebox_calls)}: `{STEP_INPUTS_METHOD}` must not call `{WHITEBOX_HELPER}`"
            )
        if source_branch_lines:
            errs.append(
...
def main() -> int:
    ...
    ap = argparse.ArgumentParser(
        description=(
            ...
            "on pretrain_contact and prevent whitebox/fallback reintroduction."
        )
    )
    ap.add_argument(
        "--training-file",
