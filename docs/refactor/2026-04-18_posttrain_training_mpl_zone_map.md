...
| Checkpoint / build-state shell | `train/posttrain_build_shell.py:108`, `train/posttrain_build_shell.py:269`, `train/posttrain_build_shell.py:346`, `train/posttrain_build_shell.py:441` | 从 checkpoint 推断 build-state，实例化 model，应用 compat load 和 selective runtime guard。 | `train.checkpoint.compat`, `torch.load`, `train.models` | `PostTrainModelBuildState`, `state_dict`, `ckpt_posttrain_cfg`, inferred head/runtime dims | 与 basetrain 共享低层 compat helper，但高层流程必须 keep-local。 |
...
| Model build / runtime prepare | `train/training_MPL.py:3404`, `train/training_MPL.py:3497`, `train/training_MPL.py:3581`, `train/training_MPL.py:3631` | 构建 `EventMotionModel`、attach encoder bundle、resume weights、history runtime。 | `train.models`, `train.checkpoint.compat`, `train.history` | `DirectPoseBuildOptions`, model kwargs, attached frozen encoder/runtime | 与 posttrain 的 checkpoint-derived build shell 共用 constructor vocabulary，但不共用 orchestration。 |
...
| checkpoint load / build-state resolution | 只有低层 seam | 低层共享于 `train.checkpoint.compat` / bundle attach；高层 orchestration 分别在 `train/posttrain_build_shell.py:108` 和 `train/training_MPL.py:3497`/`train/training_MPL.py:3631` | 不应把“从 checkpoint 反推 model build-state”和“basetrain resume load”抽成一个共同入口。 |
