...
> RootYaw 已废弃：训练/转换流程不再写入或使用 RootYaw，骨盆朝向直接通过 rot6d geodesic 学习；原始 JSON 中的 RootYaw 若存在会被忽略。
...
> ⚠️ RootYaw 已弃用：不再写入 NPZ 或参与训练/推理。如需诊断 yaw，请直接从 rot6d 还原。
...
> 说明：RootYaw 不再写入 NPZ；若需要 yaw，可在评估时从 rot6d 直接推回。
...
#### 相对化重投影（Condition Reprojection）

**新增功能 (2025-11-21)**：为了让模型学会自我纠偏，系统引入了**相对化重投影**机制。

**核心原理：**

在 Scheduled Sampling 和 Free Run 模式下，当使用模型预测而非 GT 时：
- 模型的根骨位置/朝向可能与 GT 存在偏差
- 如果目标方向始终以世界坐标系表示，模型无法"察觉"自己的偏差
- **重投影**将目标方向转换到模型的局部坐标系，让模型"看到"偏差
...
- **`enable_cond_reprojection`**: 是否启用相对化重投影（默认: `true`）
  - `true`: 在 `free`/`train_free`/`mixed` 模式下自动重投影
  - `false`: 保持原有行为，目标方向始终为世界坐标系

**生效条件：**
- 仅在 `mode` 为 `'free'`, `'train_free'`, 或 `'mixed'` 时应用
- 从第 2 帧开始（t > 0），因为第 1 帧使用 GT

**优势：**
1. **学会纠偏**：模型能够感知自身偏差，学会主动修正
2. **减少漂移**：长时序生成时，累积误差显著降低
3. **动态一致**：生成的动作更连贯，符合物理直觉

**推荐设置：**
```json
{
  "enable_cond_reprojection": true
}
```
...
#### 为什么直接用软接触（soft contacts）做预训练目标？

**本项目当前采用的方法**：`soft_contacts` 直接作为预训练的 hint/target（不再做上升沿检测、相位插值、sin/cos 映射）。

**设计考虑**：

| 方法 | 优点 | 缺点 | 泛化性 |
|------|------|------|--------|
| **软接触（本项目采用）** | 物理意义明确（任意动作均成立）<br>不依赖周期性假设<br>不会在非周期动作退化为“假相位” | 需要可靠的 soft_contact_score 标注 | ⭐⭐⭐⭐⭐ |
| 显式周期 | 训练稳定<br>收敛快 | 强依赖标注/假设<br>对非周期/过渡动作不自然 | ⭐⭐ |

**实现方式**（tanh-friendly）：
```python
# soft_contacts: [T, 2] 连续值 in [0, 1]
soft_contacts = data["soft_contacts"]

# 预训练里 period_head 输出会经过 tanh，因此把目标映射到 [-1, 1]
contact_hint = 2.0 * soft_contacts - 1.0

# 训练时对齐：soft_period[..., :2] ≈ contact_hint
```

更详细的动机、实现与指标约定见：`docs/soft_contact_hint_pretrain.md`。
...
- [ ] **软接触 Hint**：是否直接使用 `soft_contacts` 作为预训练 hint/target（避免上升沿+相位插值等人工构造）？
...
✅ 使用软接触 hint（直接 soft_contacts）而非显式周期/相位构造
✅ 零显式foot约束，完全依赖动力学学习
