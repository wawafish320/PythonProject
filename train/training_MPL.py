...
class Trainer:
    ...
    def _maybe_apply_teacher_noise(self, state_seq: torch.Tensor) -> torch.Tensor:
        """
        Inject small rotation noise on teacher inputs (DSM small-noise regime).
        """
        noise_profile = getattr(self, "input_noise_profile", None)
        noise_deg = float(getattr(self, "teacher_rot_noise_deg", 0.0) or 0.0)
        noise_prob = float(getattr(self, "teacher_rot_noise_prob", 0.0) or 0.0)
        if noise_deg > 1e-6 and noise_prob > 0.0:
            try:
                state_seq = self._apply_rot_noise(
                    state_seq,
    ...
    def _auto_tune_freerun(self, free_metrics: dict[str, float] | None, grad_info=None):
        ...
        """
        if bool(getattr(self, 'disable_auto_tune_freerun', False)):
            return None
        import math as _math
    ...
    def _apply_stage_schedule(self, epoch: int):
        ...
        # DSM: stage2 (ep 7-24) linear interpolation overrides (apply after stage params)
        if bool(getattr(self, 'stage2_interpolation', False)):
            interp = self._stage2_interpolated_params(epoch)
            if interp:
    ...
    def _stage2_interpolated_params(self, epoch: int) -> Optional[Dict[str, Any]]:
        """DSM schedule: ep 7-24 linear interpolation for key knobs."""
        if epoch < 7 or epoch > 24:
            return None
        progress = (epoch - 7) / (24 - 7)  # 0 -> 1
        return {
            'denoise_single_weight': 0.25 - 0.15 * progress,
            'freerun_weight': 0.03 + 0.09 * progress,
            'freerun_horizon': 8 + int(round(6 * progress)),
            'tf_max': 0.95 - 0.15 * progress,
            'tf_min': 0.85 - 0.20 * progress,
        }

    def evaluate_contraction(self, loader, noise_levels: list[float], max_batches: int = 2) -> dict[str, float]:
        """
        收缩比评测：对输入注入噪声，单步预测下一帧，计算输出误差/输入噪声。
        """
        import torch
        deg = 180.0 / _math.pi
        stats = []
        ...
        if not isinstance(rot_slice, slice):
            print("[ContractionEval] rot slice missing; abort.")
            return {}
        ...
        for bi, batch in enumerate(loader):
            ...
            with torch.no_grad():
                ...
                for noise_deg in noise_levels:
                    ...
                    preds, _ = self._rollout_sequence(
                        noisy,
                        cond_seq,
                        cond_raw_seq,
                    ...
                    y_pred = preds.get('out')
                    if y_pred is None:
                        continue
                    try:
                        y_pred_raw = self._denorm(y_pred)
                        y_gt_raw = self._denorm(gt_seq)
                    except Exception:
                        continue
                    geo = self._rot_geo_from_raw_seq(y_pred_raw, y_gt_raw)
                    if geo is None:
                        continue
                    out_err = geo.mean() * deg
                    stats.append({"noise_deg": float(noise_deg), "output_deg": float(out_err.cpu())})
        ...
        for s in stats:
            agg[s["noise_deg"]].append(s["output_deg"])
        result = {}
        for k, vals in agg.items():
            out = sum(vals) / len(vals)
            result[f"noise_{k}_output_deg"] = out
            result[f"noise_{k}_ratio"] = out / k
        return result
    ...
    def _apply_config_changes(self, config: Mapping[str, Any]):
        ...
        # Handle loss_groups (e.g., "core" group weights)
    ...
    def __init__(self, model, loss_fn, lr=0.0001, grad_clip=0.0, weight_decay=0.01, tf_warmup_steps=0, tf_total_steps=0, augmentor=None, use_amp=None, accum_steps=1, *, pin_memory=False, args=None):
        ...
        self.denoise_single_enabled: bool = False
        self.denoise_single_weight: float = 0.0
    ...
    def fit(self, train_loader, epochs=10, log_every=50, out_dir=None, patience=10, run_name='run'):
        ...
        for ep in range(1, int(epochs)+1):
            ...
            for bi, batch in enumerate(train_loader, start=1):
                ...
                # ---- denoise_single (DSM) ----
                denoise_enabled = bool(getattr(self, 'denoise_single_enabled', False))
                ...
                if (
                    ...
                    and gt_seq.dim() == 3
                    and gt_seq.shape[1] >= 2
                ):
                    try:
                        import torch
                        t = torch.randint(0, gt_seq.shape[1] - 1, (1,), device=gt_seq.device).item()
                        with torch.no_grad():
                            state_raw_t = self._denorm(gt_seq[:, t])
                            noisy_raw = self._apply_rot_noise(
                                state_raw_t,
                                rot_slice,
                                noise_prob=float(getattr(self, "teacher_rot_noise_prob", 1.0) or 0.0),
                                noise_deg=float(getattr(self, "teacher_rot_noise_deg", 3.0) or 0.0),
                                noise_profile=getattr(self, "input_noise_profile", None),
                                min_time_steps=1,
                            )
                        noisy_norm = self._diag_norm_x(noisy_raw)
                        cond_t = cond_seq[:, t] if cond_seq is not None else None
                        contacts_t = contacts_seq[:, t] if contacts_seq is not None else None
                        angvel_t = angvel_seq[:, t] if angvel_seq is not None else None
                        ret = self.model(noisy_norm, cond_t, contacts=contacts_t, angvel=angvel_t)
                        delta_out = ret.get('delta')
                        y_pred_raw = self._compose_delta_to_raw(noisy_raw, delta_out)
                        y_pred_norm = self._norm_y(y_pred_raw).unsqueeze(1)
                        gt_next = gt_seq[:, t + 1:t + 2]
                        denoise_loss, _ = self.loss_fn(
                            {'out': y_pred_norm, 'delta': delta_out.unsqueeze(1)},
                            gt_next
                        )
                        loss = loss + denoise_w * denoise_loss
                        stats['denoise_single_loss'] = float(denoise_loss.detach().cpu())
                        stats['denoise_single_weight'] = denoise_w
                    except Exception:
                        pass
...
def train_entry():
    ...
    p.add_argument('--freerun_noise_scale', type=float, default=0.0,
                   help='freerun 额外噪声尺度（DSM 设 0）。')
    ...
    p.add_argument('--stage2_interpolation', action='store_true', default=False,
    ...
    p.add_argument('--contraction_noise_levels', type=str, default='1,2,3,4',
                   help='收缩比评测噪声角度列表（度），逗号分隔。')
    p.add_argument('--contraction_batches', type=int, default=2,
                   help='收缩比评测最多使用的 batch 数。')
    ...
    trainer.freerun_noise_scale = float(_arg('freerun_noise_scale', 0.0) or 0.0)
    ...
    trainer.stage2_interpolation = bool(_arg('stage2_interpolation', False))
    trainer.disable_auto_tune_freerun = bool(_arg('disable_auto_tune_freerun', False))
    contraction_eval_only = bool(_arg('eval_contraction_only', False))
    contraction_noise_levels = [float(x) for x in str(_arg('contraction_noise_levels', '1,2,3,4')).split(',') if x.strip()]
    contraction_batches = int(_arg('contraction_batches', 2) or 2)
    ...
    if contraction_eval_only:
        metrics = trainer.evaluate_contraction(
            train_loader,
            noise_levels=contraction_noise_levels,
            max_batches=contraction_batches,
        )
        print("[ContractionEval]", metrics)
        return
